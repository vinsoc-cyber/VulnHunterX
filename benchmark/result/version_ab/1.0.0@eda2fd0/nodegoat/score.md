# Score — 1.0.0@eda2fd0

Model `gpt-5.5` · temp `0` · panel `sha256:1179d5607…` · 2026-07-01T06:28:53

precision **91%** · recall **62%** · TP 11 (real 10, false-alarm 1) · real 16 · not-real 1 · NMD 5 · err 0 · $3.8343
_resources:_ 447k in / 91k out · cache 57% · 3459.7s model-time · iters μ4.29

| finding | truth | verdict | grade | conf |
|---|---|---|---|---|
| js/clear-text-cookie@server.js:78 | real | NMD | abstain | High |
| js/code-injection@app/data/allocations-dao.js:78 | real | TP | CORRECT | Low |
| js/code-injection@app/routes/contributions.js:32 | real | TP | CORRECT | Medium |
| js/code-injection@app/routes/contributions.js:33 | real | TP | CORRECT | Low |
| js/code-injection@app/routes/contributions.js:34 | real | TP | CORRECT | Medium |
| js/indirect-command-line-injection@Gruntfile.js:166 | not-real | TP | FALSE-ALARM | Low |
| js/log-injection@app/routes/session.js:64 | real | TP | CORRECT | High |
| js/missing-rate-limiting@app/routes/index.js:34 | real | NMD | abstain | Medium |
| js/missing-token-validation@server.js:78 | real | FP | MISS | Medium |
| js/polynomial-redos@app/routes/profile.js:61 | real | TP | CORRECT | High |
| js/polynomial-redos@app/routes/session.js:181 | real | TP | CORRECT | Low |
| js/redos@app/routes/profile.js:59 | real | NMD | abstain | Medium |
| js/request-forgery@app/routes/research.js:16 | real | TP | CORRECT | High |
| js/server-side-unvalidated-url-redirection@app/routes/index.js:72 | real | TP | CORRECT | High |
| js/session-fixation@app/routes/index.js:34 | real | TP | CORRECT | Low |
| js/sql-injection@app/data/user-dao.js:104 | real | NMD | abstain | High |
| js/sql-injection@app/data/user-dao.js:91 | real | NMD | abstain | High |
