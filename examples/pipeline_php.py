...
Runs the full pipeline on two repos to demonstrate the contrast:
  - monolog   (real-world logging library) → expect mostly False Positives
  - dvwa      (intentionally vulnerable)    → expect True Positives
...
LANGUAGE = "php"
REPOS = [
    {"name": "monolog",  "label": "real-world logging library"},
    {"name": "dvwa",     "label": "intentionally vulnerable  "},
]
MAX_FINDINGS = 5
MAX_ITERATIONS = 5
...
def main() -> None:
    ...
    print("""
╔══════════════════════════════════════════════════════════════════════╗
║  VulnHunterX — PHP Pipeline                                          ║
║  normal:     monolog  (Seldaek logging library)                      ║
║  vulnerable: dvwa     (Damn Vulnerable Web Application)              ║
╚══════════════════════════════════════════════════════════════════════╝
""")
