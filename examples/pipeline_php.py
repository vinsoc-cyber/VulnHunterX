#!/usr/bin/env python3
"""
Full Pipeline Example: PHP Repository (dvwa)

This script demonstrates the complete CodeQL + LLM verification pipeline
for a PHP repository.

Note:
    Ensure `dvwa` is present in `config/repos.yaml` before running clone.

Usage:
    python examples/pipeline_php.py              # Run full pipeline
    python examples/pipeline_php.py --dry-run    # Preview without executing
    python examples/pipeline_php.py --skip-clone # Skip clone if already exists
    python examples/pipeline_php.py --api        # Use Python API instead of CLI
"""
...
def stage_clone(dry_run: bool = False, skip: bool = False) -> bool:
    ...
    print(f"Repository: {REPO_NAME} (Damn Vulnerable Web Application)")
    print(f"Language: {LANGUAGE}")
    print("Build: No compilation required")
    print()
...
def stage_analyze(dry_run: bool = False) -> bool:
    ...
    print("Running PHP security-extended query suite...")
    print("This includes checks for:")
    print("  - SQL injection")
    print("  - Command injection")
    print("  - Path traversal")
    print("  - XSS (Cross-Site Scripting)")
    print("  - Insecure file inclusion")
    print("  - Weak cryptographic usage")
    print()
...
def stage_extract_context(dry_run: bool = False) -> bool:
    ...
    print("Extracting PHP-specific context:")
    print("  - functions.csv: Function and method definitions")
