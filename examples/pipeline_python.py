#!/usr/bin/env python3
"""
Full Pipeline Example: Python Repository (pyyaml)

This script demonstrates the complete CodeQL + LLM verification pipeline
for a Python repository. Python projects don't require compilation,
making the database creation much faster.
...
def run_command(cmd: list[str], dry_run: bool = False) -> tuple[bool, str]:
    ...
    try:
        result = subprocess.run(
            ...
            text=True,
            timeout=600,  # 10 minute timeout (Python is fast)
        )
...
def stage_clone(dry_run: bool = False, skip: bool = False) -> bool:
    ...
    print(f"Repository: {REPO_NAME} (YAML parser for Python)")
...
def stage_analyze(dry_run: bool = False) -> bool:
    ...
    print("Running Python security-extended query suite...")
...
def stage_extract_context(dry_run: bool = False) -> bool:
    ...
    print("Extracting Python-specific context:")
...
║         Python Repository: {REPO_NAME:<42}║
╚══════════════════════════════════════════════════════════════════════╝
""")
