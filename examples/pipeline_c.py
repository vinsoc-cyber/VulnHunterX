...
Runs the full pipeline on two repos to demonstrate the contrast:
  - c-ares      (real-world DNS library)  → expect mostly False Positives
  - dvcp        (intentionally vulnerable) → expect True Positives

Note: C requires a build command for CodeQL database creation.
...
LANGUAGE = "c"
REPOS = [
    {"name": "c-ares",  "label": "real-world DNS library    "},
    {"name": "dvcp",    "label": "intentionally vulnerable  "},
]
MAX_FINDINGS = 5
MAX_ITERATIONS = 5
...
def run_fuzz(repo: str, dry_run: bool) -> dict[str, bool]:
    """Stages 5-8: fuzz confirmation (C only)."""
    results: dict[str, bool] = {}
...
║  normal:     c-ares  (DNS client library)                            ║
║  vulnerable: dvcp    (Damn Vulnerable C Program)                     ║
╚══════════════════════════════════════════════════════════════════════╝
""")
