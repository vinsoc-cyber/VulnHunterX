...
Runs the full pipeline on two repos to demonstrate the contrast:
  - commons-collections  (real-world Apache library)   → expect mostly False Positives
  - webgoat              (intentionally vulnerable app) → expect True Positives

Note: Java requires Maven or Gradle for CodeQL database creation.
...
LANGUAGE = "java"
REPOS = [
    {"name": "commons-collections", "label": "real-world Apache library "},
    {"name": "webgoat",             "label": "intentionally vulnerable  "},
]
MAX_FINDINGS = 5
MAX_ITERATIONS = 5
...
def main() -> None:
    ...
    print("""
╔══════════════════════════════════════════════════════════════════════╗
║  VulnHunterX — Java Pipeline                                         ║
║  normal:     commons-collections  (Apache utility library)           ║
║  vulnerable: webgoat              (OWASP WebGoat training app)       ║
╚══════════════════════════════════════════════════════════════════════╝
""")
